  ...
  return <EventShell>
    ...
    <div className="grid gap-8 py-8 lg:grid-cols-[minmax(0,1fr)_minmax(22rem,0.85fr)]">
      ...
      <section className="space-y-5">
        ...
        <EventPanel variant="muted" aria-label="Recovered letters"><h2 className="font-mono-data mb-4 text-xs tracking-[0.16em] uppercase text-muted-foreground">Recovered letters</h2><div className="space-y-3">{model.questions.filter((q) => q.status === "solved").map((q) => <div key={q.id} className="flex justify-between gap-3 border-b border-border pb-2 text-xs"><span className="text-muted-foreground">{q.label}</span><span className="font-mono-data text-primary">{q.codeFragment}</span></div>)}{solved === 0 ? <p className="text-sm text-muted-foreground">No letters recovered yet.</p> : null}</div></EventPanel>
