...
describe("Round1View", () => {
  it("renders fixture progression and cipher gate action", () => {
    render(<Round1View model={completeRound1Fixture} onBack={vi.fn()} onOpenGate={vi.fn()} onOpen={vi.fn()} onClaim={vi.fn()} onSelect={vi.fn()} onSubmit={vi.fn()} onRelease={vi.fn()} />);

    expect(screen.getByRole("heading", { name: /trail complete/i })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: /open cipher gate/i })).toBeInTheDocument();
  ...
  it("submits the selected answer and can release ownership", async () => {
    ...
    const { rerender } = render(<Round1View model={claimedRound1Fixture} onBack={vi.fn()} onOpenGate={vi.fn()} onOpen={vi.fn()} onClaim={vi.fn()} onSelect={onSelect} onSubmit={onSubmit} onRelease={onRelease} />);
  ...
  it("opens available clues from the sequence without claiming until the panel button is pressed", async () => {
    ...
    const onOpen = vi.fn();
    const onClaim = vi.fn();
    render(<Round1View model={{ ...availableRound1Fixture, busy: false }} onBack={vi.fn()} onOpenGate={vi.fn()} onOpen={onOpen} onClaim={onClaim} onSelect={vi.fn()} onSubmit={vi.fn()} onRelease={vi.fn()} />);
  ...
  it("lets participants switch between their claimed clues without the broken inline claim look", async () => {
    const user = userEvent.setup();
    const onOpen = vi.fn();
    render(<Round1View model={claimedRound1Fixture} onBack={vi.fn()} onOpenGate={vi.fn()} onOpen={onOpen} onClaim={vi.fn()} onSelect={vi.fn()} onSubmit={vi.fn()} onRelease={vi.fn()} />);
